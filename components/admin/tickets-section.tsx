...
export function TicketsSection() {
  const { data: tickets, setData, loading, error: ticketsError, refetch } = useTickets();
  ...
  const handleUpdateTicket = async (field: "priority" | "status", value: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // Update the ticket in the main list, merging the changes
        // to ensure object shape consistency and avoid re-render issues.
        setData(currentTickets => 
          currentTickets.map(ticket => {
            if (ticket.id === updatedTicketFromServer.id) {
              return {
                ...ticket, // Keep the original ticket's shape
                status: updatedTicketFromServer.status,
                priority: updatedTicketFromServer.priority,
                updatedAt: updatedTicketFromServer.updatedAt,
              };
            }
            return ticket;
          })
